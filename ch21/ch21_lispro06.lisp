...
;대기 표현 밑에는 보다 일반적인 abitrator 이다. 이 함수는 현재 프로세스를 저장하고 (아마 같은) 어떤 프로세스를 다시 실행하기 위해 pick-process를 호출한다.  그것은 두 개의 인수를 받는다 : 테스트 기능과 연속. 전자는 일시정지된 프로세스의 proc-wait에 저장될 것이고, 다시 시ㅏㄹ될 수 있을 때 결정되도록 추후에 호출된다. 후자는 proc-state가 되고, 그것을 호출하는 것은 일시중지된 프로세스를 다시 시작하는 것을 호출한다.
