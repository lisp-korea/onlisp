...
;그것은 비효율과 같습니다 REPL은 원시 목록을 넘겨하고, 어느 자리에서 그것을 컴파일하거나 통역으로 평가해야한다. 어찌 됐든 미리 코드를 컴파일하고, 그냥 전화보다 느립니다.
;표현이없이 어휘 맥락으로 평가되기 때문에 그것은 덜 강력한입니다. 다른 것들 중에는, 이것이 표현은 평가를 받고 밖으로 보이는 일반적인 변수를 참조할 수 없다는 것을 의미합니다.
;보통 REPL을 명시적으로 호출하는 것은 공항의 선물 가게에서 뭔가를 구입 같다. 마지막 순간까지 기다렸다가 먹고, 당신은 이류 상품의 제한된 선택을 위해 높은 가격을 지불하셔야합니다.
